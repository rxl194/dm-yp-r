...
```{r overall topic model ,echo=FALSE,fig.width=8,fig.height=8,fig.cap="Figure 2. A topic model for Yelp restaurant reviews. Color luminance shows the relative in-topic relevance of each term, being the most bright the most frequent term."}
...
Since the topic model shows some topics related to cuisine types, I have investigated how that relates to the number of restaurants that belong to a certain cuisine. Figure 3 displays the relative relevance of each cuisine category in the business data. As we can see, the topics found match the most frequent cuisine types.


```{r cuisine word cloud,fig.width=6,fig.height=6,echo=FALSE, fig.cap= "Figure 3. Word cloud displaying relevance of different cuisine categories."}
...
The overall topic model lists several topics about customer experience. But good and bad experiences are mixed because the corpus includes both positive and negative reviews. I have explored the topics related to positive and negative ratings independently. Figure 4 displays the restaurant review rating distribution. We can see that positive reviews (stars >3) dominate over negative reviews (stars <3).

```{r ratings distribution ,echo=FALSE,fig.width=4,fig.height=4, fig.cap="Figure 4. Restaurant review ratings distribution."}
...
I have fitted two topic model (one for positive reviews and another for negative reviews) with 20 topics each and following the same methodology that I used to compute the overall topic model. I didn’t include reviews with three stars because that rating is not positive nor negative. To avoid an unbalanced number of reviews in each category, I used a random sample for the positive category with size equal to the number of reviews in the negative category.
...
```{r,echo=FALSE,fig.width=8,fig.height=8,fig.cap="Figure 5. A topic model for positive Yelp restaurant reviews. Color luminance shows the relative in-topic relevance of each term, being the most bright the most frequent term."}
...
```{r,echo=FALSE,fig.width=8,fig.height=8,fig.cap="Figure 6. A topic model for Yelp restaurant reviews. Color luminance shows the relative in-topic relevance of each term, being the most bright the most frequent term."}
...
The topics found display themes related to different cuisines and customer experience. The cuisine types found correspond to the most frequent cuisine types listed in the business data.
...
Finally, I want to point out that, although all the reviews used belong to businesses labeled in Yelp as restaurants, actually they are not. Two topics show this, and further steps should be taken to clean the corpus.
