...
train_file <- 'results/dish_discovery/chinese/chin_w2v_train.txt'

output_file_bin <- 'results/dish_discovery/chinese/chin_w2v_model.bin'

output_file_text <- 'results/dish_discovery/chinese/chin_w2v_model.txt'

output_file_bigram <- 'results/dish_discovery/chinese/chin_w2v_bigram.txt'

output_file_trigram <- 'results/dish_discovery/chinese/chin_w2v_trigram.txt'
...
chinese <-data.frame(text=readLines(f),stringsAsFactors = FALSE)
...
labels <- read.table('results/manualAnnotationTask/Chinese.label',sep="\t",stringsAsFactors = FALSE,quote="\"") %>% filter(V2==1) %>% mutate(V1=gsub(" ","_",V1)) %>% filter(V1 %in% rownames(cosine.similarity))
...
f <- file("results/dish_discovery/chinese/chinese_dishes.txt","w")
