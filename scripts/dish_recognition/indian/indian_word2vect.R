...
train_file <- 'results/dish_discovery/indian/ind_w2v_train.txt'

output_file_bin <- 'results/dish_discovery/indian/ind_w2v_model.bin'

output_file_text <- 'results/dish_discovery/indian/ind_w2v_model.txt'

output_file_bigram <- 'results/dish_discovery/indian/ind_w2v_bigram.txt'

output_file_trigram <- 'results/dish_discovery/indian/ind_w2v_trigram.txt'
...
indian <-data.frame(text=readLines(f),stringsAsFactors = FALSE)
...
labels <- read.table('results/manualAnnotationTask/Indian.label',sep="\t",stringsAsFactors = FALSE,quote="\"") %>% filter(V2==1) %>% mutate(V1=gsub(" ","_",V1)) %>% filter(V1 %in% rownames(cosine.similarity))
...
dishes <- extract.related.words(cosine.similarity,labels,threshold = 0.9,max.depth = max.depth)


f <- file("results/dish_discovery/indian/indian_dishes.txt","w")
