...
train_file <- 'results/dish_discovery/mediterranean/medit_w2v_train.txt'

output_file_bin <- 'results/dish_discovery/mediterranean/medit_w2v_model.bin'

output_file_text <- 'results/dish_discovery/mediterranean/medit_w2v_model.txt'

output_file_bigram <- 'results/dish_discovery/mediterranean/medit_w2v_bigram.txt'

output_file_trigram <- 'results/dish_discovery/mediterranean/medit_w2v_trigram.txt'
...
mediterranean <-data.frame(text=readLines(f),stringsAsFactors = FALSE)
...
f <- file(train_file)
writeLines(mediterranean$text,f)
close(f)
...
labels <- read.table('results/manualAnnotationTask/Mediterranean.label',sep="\t",stringsAsFactors = FALSE,quote="\"") %>% filter(V2==1) %>% mutate(V1=gsub(" ","_",V1)) %>% filter(V1 %in% rownames(cosine.similarity))
...
dishes <- extract.related.words(cosine.similarity,labels,threshold = 0.8,max.depth = max.depth)




f <- file("results/dish_discovery/mediterranean/mediterranean_dishes.txt","w")
