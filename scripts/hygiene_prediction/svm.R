...
submission.prediction <- as.character(as.numeric(predict(svmFit,newdata=test) >=0.5))


sum(submission.prediction=="1")/length(submission.prediction)

save(svmFit,submission.prediction,train.set,test.set,test,file="results/hygiene_prediction/svm_dtm_topic_model_50.RData")
